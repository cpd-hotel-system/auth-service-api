...
import com.cpd.hotel_system.auth_service_api.repo.SystemAvatarRepo;
import com.cpd.hotel_system.auth_service_api.repo.SystemUserRepo;
import com.cpd.hotel_system.auth_service_api.service.FileService;
import com.cpd.hotel_system.auth_service_api.service.SystemAvatarService;
import com.cpd.hotel_system.auth_service_api.util.CommonFileSavedBinaryDataDTO;
...
public class SystemAvatarServiceImpl implements SystemAvatarService {
    ...
    public void createSystemUserAvatar(MultipartFile file, String email) throws SQLException {
        ...
        Optional<SystemAvatar> selectedAvatar = systemUserAvatarRepo
                .findByUserPropertyId(selectedUser.get().getUserId());
        if (selectedAvatar.isPresent()) {
            try {
                try {

                    // Delete the existing avatar resource directory
                    fileService.deleteResource(bucketName, "avatar/" + selectedUser.get().getUserId() + "/resource/",
                            fileDataExtractor.byteArrayToString(selectedAvatar.get().getFileName()));
                } catch (Exception e) {
                    // Handle deletion failure if needed
                    throw new InternalServerException("Failed to delete existing avatar resource directory");
                }

                resource = fileService.createResource(file, "avatar/" + selectedUser.get().getUserId() + "/resource/",
                        bucketName);
